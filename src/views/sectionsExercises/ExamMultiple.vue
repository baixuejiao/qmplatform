...
<template>
    <div class="clearfix">
        <div class="exam-box">
            ...
            <div :class="{'question-box':true,'clearfix':true,'slide':slide}">
...
export default {
    ...
    methods:{
            ...
            if(this.isAnswerd){  // 未答题
                ...
                if(!isArrayEqual(this.allAnswers,arr)){
                    ...
                    this.$refs.message.show({
                        message:{title:'对不起回答错误，正确答案是：'+arr.join(''),content:this.nowQuestion.answerDesc},
                        type:'error'
                    });
                    this.status = 0;
                }else{
                    ...
                    this.status = 1;
                    this.$refs.message.show({
                        message:{title:'恭喜你回答正确，正确答案是：'+arr.join(''),content:this.nowQuestion.answerDesc},
                        type:'success'
                    });
                }
                // 当前题已答
        ...
        nextQuestion(){
            if(!this.isAnswerd){ //如果当前题答完
                ...
                this.allAnswers = [];
                // 重新开启定时器
                this.$refs.countTimer.openTimeCount(30);
            }
    ...
    computed:{
        // 下一题 还是完成
        next(){
            return this.nowIndex == this.questionNum - 1 ? '完成':'下一题';
        }
    },
