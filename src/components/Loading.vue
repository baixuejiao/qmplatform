<!--
  加载框
  全局调用方式：
      打开--》this.$store.commit("showLoading")
      关闭--》this.$store.commit("hideLoading")
  组件引用方式：
  props: show 接受布尔值 是否显示
         type 接受字符型 加载动画类型 1-19
         text 接受字符型 加载框显示的文字 
-->
<template>
  <div class="loading-mosk" v-if="isShow">
    <div v-for="item in 19" :key="item">
      <div v-if="item==loadingType" class="box">
        <div :class="'loader-'+(item>9?'':'0')+item"></div>
...
.box {
  ...
  animation: loadingrun 3s ease infinite;
  position: absolute;
  top: 50%;
  left: 50%;
}
